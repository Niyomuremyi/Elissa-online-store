...
function saveCart() {
  localStorage.setItem("elissaCart", JSON.stringify(cart));
}
function cartCount() {
...
function initNav() {
  ...
  window.addEventListener(
    ...
    () =>
      (nav.style.boxShadow =
        scrollY > 40 ? "0 4px 20px rgba(0,0,0,.5)" : "none"),
  );
...
function initAnim() {
  ...
  document
    .querySelectorAll(
      ".product-card,.cat-card,.team-card,.perk,.a-box,.flag-item",
    )
    .forEach((el) => {
      el.style.opacity = "0";
...
function initNewsletter() {
  ...
  f.addEventListener("submit", (e) => {
    ...
    if (inp.value) {
      showToast("🎉 Subscribed! Murakoze (Thank you)!");
      inp.value = "";
    }
  });
}
